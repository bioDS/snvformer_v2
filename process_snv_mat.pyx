...
def enc_v5(string_to_tok, tok_to_string, pos, p: int, a0, a1):
    a0_toks  = np.array(np.repeat(pos+0, len(a0)), dtype=np.int32)
    a01_toks = np.array(np.repeat(pos+1, len(a0)), dtype=np.int32)
    a1_toks  = np.array(np.repeat(pos+2, len(a0)), dtype=np.int32)
    for n in [0, 1, 2]:
        tok_to_string[pos+n] = str(n)
        string_to_tok[str(n)] = pos+n
    return a0_toks, a1_toks, a01_toks, tok_to_string, string_to_tok
...
def get_tok_mat(geno, encoding: int = 2):
    ...
    for batch in tqdm(range(int(np.ceil(float(n)/batch_size)))):
        # don't convert to int, there are 'nan's in the data.
        submat = np.array(geno[batch*batch_size:(batch+1)*batch_size].values, dtype=np.float32)
        submat_view = submat
        actual_batch_len = submat.shape[0]
        with nogil:
