...
def hyena_huggingface_model(pretrained_model_name="hyenadna-tiny-1k-seqlen"):
...
def get_hyena_layer(args):
    ...
    return hyena_dna_layer

def get_model(args):
    num_snv_classes = get_num_snv_classes(args.h5_file)
    if args.pretrained_hyena != None:
        encoder = hyena_huggingface_model(args.pretrained_hyena)
    else:
        encoder = HyenaDNAModel(
            d_model = args.d_model, # d_model (int): Dimension of the input and output embeddings (width of the layer)
            n_layer = args.num_layers,
            d_inner = 4 * args.d_model, # default in code
            vocab_size = num_snv_classes,
            layer = get_hyena_layer(args),
...
def main(args):
    ...
    trainer = HyenaEncoderTrainer(model,
                                train_data,
                                mini_test_data,
                                verify_data,
                                optimizer,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--devices', help='comma delimited list of gpus to use', type=str, default="4,5,6,7")
